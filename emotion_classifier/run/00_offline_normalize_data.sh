...
usage() {
  cat <<'EOF'
Usage:
  ./run/00_offline_normalize_data.sh --input RAW_FILE [--output NORMALIZED_FILE]
  ./run/00_offline_normalize_data.sh --hf-source HF_SOURCE [--output NORMALIZED_FILE]
...
if [[ -z "${OUTPUT_PATH}" ]]; then
  OUTPUT_PATH="$(make_data_output_path normalized "${SOURCE_RESOLVED}" parquet)"
fi
