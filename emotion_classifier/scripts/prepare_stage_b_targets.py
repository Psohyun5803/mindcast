...
def main() -> None:
    ...
    if "base_emotion_label" not in df.columns:
        raise ValueError("Expected base_emotion_label or an alias such as comment_only_pred in the input dataset")
    ...
    work["actual_emotion_label"] = work.get("actual_emotion_target", "")
    work["actual_emotion_label"] = work["actual_emotion_label"].fillna("").astype(str).str.strip()
    ...
    print(
        json.dumps(
            {
                "rows": int(len(work)),
                "sarcasm_positive": int(work["sarcasm_label"].sum()),
