...
├── src/suicide_utils.py          # 전처리·모델·학습·평가 함수 전체
├── pipeline/suicide_pipeline.py  # 실행 파이프라인 (CLI)
└── run/suicide_run.sh            # 쉘 실행 스크립트

root/data/suicide_predict/
├── cache/
│   ├── raw/                      # target parquet, splits.json
│   ├── emotion/                  # KOTE 추론 결과
│   └── topic/                    # 토픽 피처
├── outputs/
│   ├── checkpoints/              # 모델 가중치 (.pt)
│   ├── predictions/              # ablation 예측 결과 parquet
│   └── figures/                  # 시각화
├── deliverable/                  # 월별 예측기 산출물 (그래프·모델카드·CSV)
└── prediction/
    ├── main/                     # 일별 inference 결과 (main-infer)
    └── monthly/                  # 월별 inference 결과 (monthly-infer)
```
...
# ── 일별 inference (학습된 main_model.pt 이용) ───────────────────
./suicide_run.sh main-infer 2024-01-01               # 하루
./suicide_run.sh main-infer 2024-01-01 2024-01-31    # 기간
...
python suicide_pipeline.py --run main-infer --start 2024-01-01 --end 2024-01-31
python suicide_pipeline.py --run monthly-infer --start 2024-03
```
...
| 단계 | 함수 | 입력 | 출력 |
|------|------|------|------|
| 1/4 target 준비 | `build_target()` | HuggingFace `MindCastSogang/SuicideDataset` | `cache/raw/target_call_counts.parquet` |
...
> **전처리 없이 다른 단계 실행 시** 누락 파일 목록과 함께 한국어 오류 메시지가 출력됩니다.
...
└── cache/
    ├── raw/
    │   ├── target_call_counts.parquet         ← 일별 상담건수
    │   ├── youtube_news_comments.parquet       ← 댓글 원본 (date, title, comment_id, ...)
    │   └── splits.json                         ← train/valid/test 날짜 구간
    ├── emotion/
...
**Setting A~F (누적 피처 추가):**
```
A: 상담 이력만                           ← 최종 채택 (MASE 최저)
B: A + 캘린더 피처 (요일/월/공휴일/sin-cos)
C: B + 댓글 볼륨 (일별 댓글량)
D: C + KOTE 감정 (4 대분류: 기쁨/슬픔/분노/중립)
E: D + 뉴스 토픽 (K=10)
F: E + 감정×토픽 상호작용 (4×10=40채널)
```

**head 모드:**
- `nb` (기본): Negative Binomial 헤드, NB-NLL 손실, 확률적 예측구간
- `point`: Huber 손실, split-conformal 예측구간 (MAIN_CFG 채택)

**산출물:**
```
root/data/suicide_predict/outputs/
├── metrics/
...
**split-conformal 보정:** seed=42 valid 절대잔차 퍼센타일로 q80/q90 계산 → 체크포인트에 함께 저장
...
└── checkpoints/
    └── main_model.pt                 ← 최종 모델 (state_dict + cfg + meta + q80 + q90)
...
### Pipeline 2.5 — 일별 inference (main-infer)

`main_model.pt`를 이용해 임의 날짜/기간에 대한 상담건수를 예측합니다.

```bash
./suicide_run.sh main-infer 2024-01-01              # 하루
./suicide_run.sh main-infer 2024-01-01 2024-12-31   # 연간
```

**input 데이터 검사:** 예측 대상일 직전 데이터가 없으면 예측하지 않고 오류 메시지 출력
- `lookback`(56일) 데이터 미달 → 해당 날짜 skip
- 마지막 데이터가 `tdate - 1일`보다 오래됨 → 즉시 중단 (preprocess 재실행 안내)

**산출물:**
```
root/data/suicide_predict/prediction/main/
└── infer_{start}_to_{end}.csv
    ← date, pred_mean, lo80, hi80, lo90, hi90
```
...
├── monthly_predictions_detail.csv    ← 월×모델 예측값 테이블 (월별 실제값 포함)
└── 14_monthly_suicide_7models.png    ← R²/MAE 막대그래프 비교
```

---

### Pipeline 3.5 — 월별 inference (monthly-infer)

y_true가 아직 없는 미래 월에 대해 자살자수를 예측합니다.

```bash
./suicide_run.sh monthly-infer 2024-03
```

**피처 가용성 자동 판단:** S/E/T 중 가용한 것만으로 조합을 구성합니다.
- S(사회경제): 정부 통계 지연으로 최신 월이 없을 수 있음
- E/T(댓글): 로컬 `comment_kote_probs.parquet` 기준

**산출물:**
```
data/suicide_predict/prediction/monthly/
└── infer_{YYYY-MM}.csv
    ← model, pred_자살자수
```
...
```
입력 모달리티              shape      DLinearBlock(L→H)
────────────────────────   ───────    ─────────────────
y_past  (상담건수 이력)    56×1    →  trend+seasonal 분해 후 linear projection
v_past  (댓글 볼륨)        56×1    →  (ablation C 이상)
e_past  (감정 4그룹)       56×4    →  (ablation D 이상)
t_past  (뉴스 토픽)        56×10   →  (ablation E 이상)
inter   (감정×토픽)        56×40   →  emo_group×topic_ratio (ablation F)
c_future(예측일 캘린더)    24      →  cal_mlp: Linear(24→32)→ReLU (leakage 없음)

모든 과거 시계열은 동일하게 56일 룩백 적용.

→ Fusion MLP → mu_head + r_head
→ mu = y_base × exp(mu_head)   ← 앵커: 최근 7일 평균
```

### RidgeCV (월별 예측기)

- StandardScaler + RidgeCV(alphas 25개)
- Expanding window (min_train=18개월)
- 예측 시점의 직전까지만 학습 (look-ahead 없음)
...
| 앵커 수준 | 최근 7일 raw 평균 |
| prediction/main | `data/suicide_predict/prediction/main/` |
| prediction/monthly | `data/suicide_predict/prediction/monthly/` |
...
[HuggingFace: MindCastSogang/Youtube_news_preprocessed_data]
  ...
  target_call.pq     youtube_comments.pq
   ...
   ┌────┴───────────────────┘
   │
   ├── [일별] dataset → train → ablation → main_model.pt
   │                                           │
   │                                    main-infer → prediction/main/
   │
   └── [월별] load_socio + monthly_emotion + monthly_topic → RidgeCV
                                                               │
                                            monthly          deliverable/
                                            monthly-infer → prediction/monthly/
```
...
다른 단계 실행 시 아래와 같은 오류가 출력됩니다:

```
[ERROR] 전처리 파일이 없습니다. 먼저 preprocess를 실행하세요:
  ./suicide_run.sh preprocess

  누락된 파일:
  emotion_labels.json
  target_call_counts.parquet
```

### main_model.pt 없이 main-infer 실행 시

```
[ERROR] main_model.pt가 없습니다. 먼저 main을 실행하세요:
  ./suicide_run.sh main
```
...
- `monthly-infer`에서 S(사회경제) 데이터는 정부 통계 지연으로 최신 월이 없을 수 있음 — 가용한 피처로만 예측
- `dataset.py` 정규화 stats는 train rows로만 fit (leakage-safe)
- `topic_features()` TF-IDF/KMeans는 TRAIN_END(2021-12-31) 이전으로만 fit (leakage-safe)
- 월별 예측기 토픽 클러스터링은 전체 데이터로 fit 
